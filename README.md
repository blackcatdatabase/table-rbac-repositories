...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 1 outbound / 3 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 2 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\rbac-repositories](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\rbac-repositories)
...
graph LR
  ...
  rbac_repositories["rbac_repositories"]:::center
  rbac_repositories -->|FK| signing_keys["signing_keys"]:::link
  rbac_repo_snapshots["rbac_repo_snapshots"]:::inbound -->|FK| rbac_repositories
  rbac_roles["rbac_roles"]:::inbound -->|FK| rbac_repositories
  rbac_sync_cursors["rbac_sync_cursors"]:::inbound -->|FK| rbac_repositories
...
| Inbound links (tables depending on this) | 3 |
| Views | 4 |
| Seeds | 0 |
| Drift warnings | 0 |
| PII flags | 2 |
...
- Outbound (depends on): "signing_keys"
- Inbound (relies on this): "rbac_repo_snapshots", "rbac_roles", "rbac_sync_cursors"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package rbac-repositories -Apply
