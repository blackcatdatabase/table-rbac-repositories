...
namespace BlackCat\Database\Packages\RbacRepositories\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacRepositories\Definitions;
use BlackCat\Database\Packages\RbacRepositories\Criteria;
use BlackCat\Database\Packages\RbacRepositories\Dto\RbacRepositoryDto as Dto;
use BlackCat\Database\Packages\RbacRepositories\Mapper\RbacRepositoryDtoMapper as RowMapper;
...
final class RbacRepositoryRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacRepositories\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacRepositories\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'url', 'signing_key_id', 'status', 'last_synced_at', 'last_commit' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRepositories\Dto\RbacRepositoryDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRepositories\Dto\RbacRepositoryDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacRepositories\Mapper\RbacRepositoryDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRepositories\Dto\RbacRepositoryDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRepositories\Dto\RbacRepositoryDto|null {
