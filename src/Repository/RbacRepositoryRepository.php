...
use BlackCat\Database\Packages\RbacRepositories\Repository\RbacRepositoryRepositoryInterface;
...
class RbacRepositoryRepository implements RbacRepositoryRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacRepositories\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'url', 'signing_key_id', 'status', 'last_synced_at', 'last_commit' ]);
          return count($rows);
        }
