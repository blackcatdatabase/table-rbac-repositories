...
namespace BlackCat\Database\Packages\RbacRepositories\Joins;
 ...
 */
final class RbacRepositoriesJoins {
    ...
    /**
     * FK: rbac_repositories -> signing_keys
     * LEFT JOIN vw_signing_keys AS $as ON $as.id = $alias.signing_key_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinSigningKeys(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_signing_keys AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.signing_key_id' . ' ', []];
    }
