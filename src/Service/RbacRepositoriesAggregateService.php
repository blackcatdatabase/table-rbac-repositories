...
namespace BlackCat\Database\Packages\RbacRepositories\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacRepositories\Dto\RbacRepositoryDto;
use BlackCat\Database\Packages\RbacRepositories\Mapper\RbacRepositoryDtoMapper;
 ...
 */
final class RbacRepositoriesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacRepositoryRepository $rbacRepositoryRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_repositories' => $this->rbacRepositoryRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacRepositories\ModuleException("Record {$idStr} not found for locking.");
            }
